...
START_TEST(compliment_test_1x1) {
  ...
  ck_assert_int_eq(s21_eq_matrix(&R, &B), FAILURE);
  s21_remove_matrix(&A);
  s21_remove_matrix(&B);
  s21_remove_matrix(&R);
}
...
START_TEST(null_compliments) {
  ...
  ck_assert_int_eq(res, BAD_MATRIX);
}
...
START_TEST(create_test_incorrect) {
  ...
  ck_assert_int_eq(code, BAD_MATRIX);
...
START_TEST(incorrect_matrix_2) {
  ...
  matrix_t result = {0};
  int mul_code = s21_mult_matrix(&m1, &m2, &result);
  ck_assert_int_eq(mul_code, BAD_MATRIX);
...
START_TEST(transpose_incorrect) {
  ...
  ck_assert_int_eq(ret, BAD_MATRIX);
  s21_remove_matrix(&A);
  s21_remove_matrix(&B);
}
